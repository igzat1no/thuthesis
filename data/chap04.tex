...
理论上说，我们期望用我们自己实现的光线追踪器完成数据的渲染过程。这是因为渲染并没有理论上的正确与错误之分，生成数据与训练使用相同的渲染器可以保证渲染过程的一致性，从而尽可能还原真实材质。然而，由于我们的光线追踪器的实现尚未完全，我们暂时使用了 Blender \cite{blender} 进行重新渲染。Blender 渲染的计算方式和我们渲染器计算方式的不同可能会对最终结果产生一定的影响，我们在\ref{sec:material} 中讨论这一点。
...
\subsection{材质相似性指标（MAT）} \label{sec:material}
...
我们首先根据真实材质图对我们得到的漫反射颜色进行修正。我们计算两者的平均颜色，然后将我们得到的漫反射颜色乘以两者的比值。在统一二者均值后，我们计算两者的 PSNR 作为材质相似性指标。具体来说，假定 $I$ 和 $I_0$ 分别是我们得到的漫反射颜色和真实材质图，我们计算：
\begin{align}
  &I' = I \times \frac{\text{mean}(I_0)}{\text{mean}(I)}, \\
  &\text{MAT}(I, I_0) = \text{PSNR}(I', I_0).
\end{align}

然而，比较我们还原得到的材质图片与真实材质图片来计算还原准确度是不完全合理的。我们使用 Blender 渲染得到数据，可以近似理解为 $\text{Blender}(G, M, L) = I_0$。我们用自己实现的渲染器进行训练，因此对于我们还原的 $G', M', L'$，实际上有 $\text{OurRenderer}(G', M', L') = I$。我们通过逼近 $I \rightarrow I_0$ 进行训练，但由于渲染器近似但并不完全相同，直接比较 $M$ 和 $M'$ 并不能合理地说明我们还原材质的真实程度。因而，完成我们的渲染器并重新生成数据是非常必要的。

不过，由于我们在实现渲染器的时候参考了 Blender 的材质模型，MAT 指标也具有一定说服力。事实上，最好的衡量还原材质真实性的方法是人类主观评价，但这种方法难以量化，因此我们采用了 MAT 指标。
...
\newpage
可以看到，引入第三阶段训练后，材质相似性指标有一定提升。我们可以通过图\ref{fig:albedo} 更为明显的看到这一点。图中的三列从左至右分别代表 \textbf{不加入第三阶段训练的漫反射颜色图像}、\textbf{真实漫反射图像}和 \textbf{加入第三阶段训练的漫反射颜色图像}。第一排是模型训练得到的结果，第二排则是根据真实漫反射颜色图像进行颜色修正后的结果。可以看到，第一张图像中两条热狗之间以及盘子下方明显的阴影均已被减弱乃至消除。
\begin{figure}
  \centering
